...
describe('pending record bookkeeping', () => {
  ...
  it('keeps modes separate so one collection cannot pin another', () => {
    const storage = fakeStorage();
    recordLocalChanges('tasks', { edited: ['1'] }, storage);
    recordLocalChanges('jobseeker', { edited: ['2'] }, storage);
    expect(sorted(readPending('tasks', storage).edited)).toEqual(['1']);
    expect(sorted(readPending('jobseeker', storage).edited)).toEqual(['2']);
  });
