    ...
    <div className="flex flex-col h-dvh bg-gray-50 font-sans" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <header className={`bg-gradient-to-r ${
        ...
        <div className="flex px-2 sm:px-6 gap-0.5 sm:gap-2 mt-2 overflow-x-auto scrollbar-none">
          {[
            { key: 'board',    icon: <Layout size={15} />,   short: t('tabs.boardShort', 'Board')    },
            { key: 'list',     icon: <List size={15} />,     short: t('tabs.listShort', 'List')      },
            { key: 'timeline', icon: <Clock size={15} />,    short: t('tabs.timelineShort', 'Timeline') },
            { key: 'calendar', icon: <Calendar size={15} />, short: t('tabs.calendarShort', 'Calendar') },
            { key: 'stats',    icon: <BarChart2 size={15} />, short: t('tabs.statsShort', 'Stats')   },
          ].map(({ key, icon, short }) => (
            <button
              key={key}
              onClick={() => navigateTo(key)}
              className={`px-2.5 sm:px-4 py-2 rounded-t-lg font-medium flex items-center gap-1.5 transition-colors whitespace-nowrap flex-shrink-0 text-sm ${activeTab === key ? 'bg-gray-50 text-indigo-800' : 'bg-white/10 text-blue-100 hover:bg-white/20 active:bg-white/30'}`}
            >
              {icon}
